...
/*   Updated: 2017/04/14 05:22:45 by apoisson         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "get_next_line.h"

static t_buff	*new_buff(int fd, int add, t_buff **buff)
{
	t_buff	*new;
...
static int		read_line(int fd, t_buff *buff, char **line, int verif)
{
	int		r;
	int		i;
...
static int		check_mem(int fd, t_buff *buff, char **line)
{
	int		i;
...
static t_buff	*check_buff(int fd, t_buff **copy)
{
	while (*copy)
	{
		if ((*copy)->fd == fd)
